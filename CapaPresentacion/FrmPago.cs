...
namespace CapaPresentacion
{
    public partial class FrmPago : Form
    {
        ...
        private void FrmPago_Load(object sender, EventArgs e)
        {
            ...
            List<Pago> listaMenu = new CN_Pago().Listar();

            // Recorre los roles de categorias existentes para elegir al momento de crear un empleado
            foreach (Pago menu in listaMenu)
            {
                gridCategorias.Rows.Add(new object[] {"", menu.Id_pago, menu.Nombre,
                           menu.Activo == 1 ? 1 : 0,
        ...
        private void btnguardar_Click(object sender, EventArgs e)
        {
            ...
            Pago pago = new Pago()
            {
                Id_pago = Convert.ToInt32(textId.Text),
            ...
            if (pago.Id_pago == 0)
            {
                ...
                int registroCategoria = new CN_Pago().Registrar(pago, out mensaje);
            ...
            {
                bool resultado = new CN_Pago().Editar(pago, out mensaje);
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Desea eliminar el Pago?", "Alerta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    string mensaje = string.Empty;
                    ...
                    bool respuesta = new CN_Pago().Eliminar(pago, out mensaje);

                    if (respuesta)
                    {
                        foreach (DataGridViewRow row in gridCategorias.Rows)
                        {
                            if (Convert.ToInt32(row.Cells["Id"].Value) == pago.Id_pago)
                            {
                                row.Cells["EstadoValor"].Value = pago.Activo.ToString();
